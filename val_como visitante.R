...
df <- players_superM %>%
  filter(nameTeam %in% jor$visitante & cancha == "away") %>%
  ...
  left_join(., jor %>% select(nameTeam = visitante, against = local), join_by(nameTeam)) %>%
  ...
  left_join(recPosloc_df %>% filter(eq %in% jor$local_abb) %>% rename(abb_against = eq), join_by(abb_against, position)) %>%
  mutate(
...
path <- here::here("2025", "superManager", "basicos", "laboratorio valoracion como local y visitante", "png", "topvalcomovis_19")
...
df %>%
  ...
  tab_header(
    title = md("<div style='line-height:134px;vertical-align:middle;text-align:left;font-weight:600;font-size:64px'>
               ...
               <span style='text-align:left;'>Top 15 Como Visitante</div>"),
    subtitle = md("<span style='font-weight:400;color:#8C8C8C;font-size:32px'>Jugadores que han disputado al menos 3 de los últimos 5 partidos de su equipo como visitante | SuperManager 24/25</span>")
  ) %>%
  tab_source_note(
  ...
  gtsave(glue::glue("visitante.png"), vwidth = 3000, vheight = 2400, expand = 200)
